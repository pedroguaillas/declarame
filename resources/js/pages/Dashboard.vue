  ...
  <AdminLayout>
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <div class="border-border flex rounded-lg border p-0.5">
        <button
          ...
          :class="activePeriod === 'month' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'"
        ...
        <button
          type="button"
          class="rounded-md px-4 py-1.5 text-sm font-medium transition-colors"
          :class="activePeriod === 'year' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'"
    ...
    <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
      ...
      <div class="border-border bg-card rounded-xl border p-5">
        <div class="mb-3 flex items-center justify-between">
          ...
          <div class="bg-blue-100 dark:bg-blue-900/30 rounded-lg p-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-blue-600 dark:text-blue-400 size-4">
        ...
        <p class="text-muted-foreground mt-1 text-sm">{{ stats.sales.count }} comprobante{{ stats.sales.count !== 1 ? 's' : '' }}</p>
      </div>
      ...
      <div class="border-border bg-card rounded-xl border p-5">
        <div class="mb-3 flex items-center justify-between">
          ...
          <div class="bg-orange-100 dark:bg-orange-900/30 rounded-lg p-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-orange-600 dark:text-orange-400 size-4">
        ...
        <p class="text-muted-foreground mt-1 text-sm">{{ stats.purchases.count }} comprobante{{ stats.purchases.count !== 1 ? 's' : '' }}</p>
      </div>
      ...
      <div class="border-border bg-card rounded-xl border p-5">
        <div class="mb-3 flex items-center justify-between">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4" :class="balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
            </svg>
          </div>
        </div>
        <p class="text-2xl font-bold tabular-nums" :class="balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        <div class="flex h-40 items-end gap-1.5">
          ...
          >
            ...
            <div class="flex h-32 w-full items-end gap-0.5">
              ...
              >
                <!-- Tooltip on hover -->
                <div class="absolute bottom-full left-1/2 mb-1 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-foreground px-2 py-1 text-xs text-background group-hover:block">
                  ${{ money(point.sales) }}
                </div>
              </div>
              <div
            ...
            <!-- Month label -->
            <span class="text-muted-foreground text-[10px]">{{ monthLabel(point.month) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="border-border bg-card lg:col-span-2 rounded-xl border p-5">
        <p class="text-foreground mb-4 text-sm font-semibold">Top proveedores este año</p>
        ...
        <ul v-else class="space-y-3">
          ...
          >
            <span class="text-muted-foreground w-4 shrink-0 text-right text-xs font-medium">{{ idx + 1 }}</span>
            ...
            <div class="shrink-0 text-right">
              <p class="text-foreground text-sm font-semibold tabular-nums">${{ money(provider.total) }}</p>
              <p class="text-muted-foreground text-xs">{{ provider.count }} doc{{ provider.count !== 1 ? 's' : '' }}</p>
            </div>
          </li>
        </ul>
      </div>
