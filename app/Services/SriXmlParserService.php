...
class SriXmlParserService
{
    ...
    private function extractIva(SimpleXMLElement $info): array
    {
        ...
        foreach ($info->totalConImpuestos->totalImpuesto as $impuesto) {
            ...
            if ($code === IVA::IVA0 || $code === IVA::IVA_EXENT0) {
                $result['base0'] += $base;
            } elseif ($code === IVA::IVA5) {
                $result['base5'] += $base;
                $result['iva5'] += $valor;
            } elseif ($code === IVA::IVA12) {
                $result['base12'] += $base;
                $result['iva12'] += $valor;
            } elseif ($code === IVA::IVA15) {
                $result['base15'] += $base;
                $result['iva15'] += $valor;
            } elseif ($code === IVA::NO_IVA) {
                $result['no_iva'] += $base;
            } elseif ($code === IVA::IVA_DIFERIDO) {
                $result['base12'] += $base;
                $result['iva12'] += $valor;
            }
        }
